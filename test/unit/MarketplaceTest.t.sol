...
contract MarketplaceTest is Test, Constants {
    ...
    // Accept an offer successfully and transfer ownership
    function testAcceptOfferAndTransferOwnership() public createListItem {
        vm.prank(buyer);
        vm.deal(buyer, OFFER_AMOUNT);
        marketplace.makeOffer{value: OFFER_AMOUNT}(LISTING_ITEM_ID_1, OFFER_AMOUNT);
        ...
        // Assert the item is marked as sold
        Marketplace.ListingItem memory listingItem = marketplace.getListingItem(LISTING_ITEM_ID_1);
        Marketplace.ListingItem[] memory buyerListingItem = marketplace.getListingsByOwner(buyer);
        Marketplace.ListingItem[] memory sellerListingItem = marketplace.getListingsByOwner(seller);
        assertEq(listingItem.sold, true);
        assertEq(listingItem.owner, buyer);
        assertEq(buyerListingItem.length, 1);
        assertEq(buyerListingItem[0].listingItemId, LISTING_ITEM_ID_1);
        assertEq(sellerListingItem.length, 0);
    }

    // Accept an offer and retrieve pending offers money
    function testAcceptOfferAndRetrievePendingOffers() public createListItem {
        uint256 initialBuyerBalance = OFFER_AMOUNT;
        uint256 initialBuyer2Balance = OFFER_AMOUNT;

        vm.prank(buyer);
        vm.deal(buyer, initialBuyerBalance);
        marketplace.makeOffer{value: initialBuyerBalance}(LISTING_ITEM_ID_1, initialBuyerBalance);

        vm.prank(buyer2);
        vm.deal(buyer2, initialBuyer2Balance);
        marketplace.makeOffer{value: initialBuyer2Balance}(LISTING_ITEM_ID_1, initialBuyer2Balance);

        vm.startPrank(seller);
        marketplace.acceptOffer(LISTING_ITEM_ID_1, OFFER_INDEX); // Accept the first offer
        vm.stopPrank();

        // Retrieve the pending offers money
        uint256 expectedBuyerBalance = initialBuyerBalance - OFFER_AMOUNT;
        uint256 expectedBuyer2Balance = initialBuyer2Balance;
        assertEq(buyer.balance, expectedBuyerBalance);
        assertEq(buyer2.balance, expectedBuyer2Balance);
    }
    ...
    // End an auction successfully and transfer ownership
    function testEndAuctionAndTransferOwnership() public startAuction {
        vm.startPrank(buyer);
        vm.deal(buyer, BUYER_INITIAL_BALANCE);
        marketplace.placeBid{value: BID_AMOUNT}(LISTING_ITEM_ID_1);
        vm.stopPrank();
    ...
    function testInsertAuctionsAndVerifyHeap() public createMultipleListItems {
        ...
        HeapUtils.AuctionStruct memory rootAuction = marketplace.getSoonestAuction();
    ...
    function testRemoveAuctionAndVerifyHeap() public createMultipleListItems {
        ...
        HeapUtils.AuctionStruct memory newRootAuction = marketplace.getSoonestAuction(); // Get the new root auction
    ...
    // Test that listings are correctly added to both the global array and the owner's array
    function testCreateListingAddsToOwnerAndAllListings() public createListItem {
        // Check global listing array
        Marketplace.ListingItem[] memory allListings = marketplace.getAllListings();
        assertEq(allListings.length, 1);
        assertEq(allListings[0].listingItemId, LISTING_ITEM_ID_1);

        // Check owner listing array
        Marketplace.ListingItem[] memory ownerListings = marketplace.getListingsByOwner(seller);
        assertEq(ownerListings.length, 1);
        assertEq(ownerListings[0].listingItemId, LISTING_ITEM_ID_1);
    }

    // Test that ownership transfer correctly updates owner listings
    function testOwnershipTransferUpdatesOwnerListings() public createListItem {
        // Buyer purchases the item
        vm.prank(buyer);
        vm.deal(buyer, BUYER_INITIAL_BALANCE); // Give buyer some Ether
        marketplace.purchaseListingItem{value: TOTAL_PRICE_WITH_FEE}(LISTING_ITEM_ID_1);

        // Check that the seller no longer has the item in their listings
        Marketplace.ListingItem[] memory sellerListings = marketplace.getListingsByOwner(seller);
        assertEq(sellerListings.length, 0); // Seller should have no listings

        // Check that the buyer now has the item in their listings
        Marketplace.ListingItem[] memory buyerListings = marketplace.getListingsByOwner(buyer);
        assertEq(buyerListings.length, 1);
        assertEq(buyerListings[0].listingItemId, LISTING_ITEM_ID_1); // Buyer now owns the item
    }
}
