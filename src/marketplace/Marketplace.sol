...
contract Marketplace is ReentrancyGuard, AutomationCompatibleInterface, Auction, Offer {
    ...
    ListingItem[] private allListings;
    mapping(address => ListingItem[]) private ownerListings;
    ...
    function createListingItem(IERC721 nft, uint256 tokenId, uint256 price) external nonReentrant {
        ...
        ListingItem memory newItem =
            ListingItem(listingItemCount, nft, tokenId, price, payable(msg.sender), msg.sender, false);
        // Add the new listing item to the listingItems mapping
        listingItems[listingItemCount] = newItem;
        // Add to the general listing array
        allListings.push(newItem);
        // Add to the owner's listings array
        ownerListings[msg.sender].push(newItem);
    ...
    function purchaseListingItem(uint256 listingItemId) external payable nonReentrant {
        ...
        // Transfer the ownership of the listing item
        _transferListingItemOwnership(listingItem.seller, msg.sender, listingItemId);
    ...
    function acceptOffer(uint256 listingItemId, uint256 offerIndex) external override nonReentrant {
        ...
        if (offerIndex >= listingOffers.length) {
            revert Offer__InvalidOfferIndex();
        }
        ...
        // Transfer the ownership of the listing item
        _transferListingItemOwnership(listingItem.seller, offer.bidder, listingItemId);

        // Remove the accepted offer
        if (listingOffers.length > 1 && offerIndex != listingOffers.length - 1) {
            listingOffers[offerIndex] = listingOffers[listingOffers.length - 1];
        }
        listingOffers.pop();

        // Refund all other offers
        _refundPendingOffers(listingItemId);
    ...
    function endAuction(uint256 listingItemId) internal override {
        ...
        if (auction.highestBidder != ZERO_ADDRESS) {
            ...
            _transferListingItemOwnership(listingItem.seller, auction.highestBidder, listingItemId);
    ...
    function getOngoingAuction(uint256 listingItemId) external view returns (HeapUtils.AuctionStruct memory) {
        ...
        return HeapUtils.AuctionStruct(0, 0, payable(address(0)), 0, 0, false);
    }
    ...
    //////////////////////////////////////////////////////////////*/
    /**
     * @notice Transfers the ownership of a listing item from one address to another
     * @dev This function updates the ownerListings mapping by removing the listing item from the current owner's list
     *      and adding it to the new owner's list.
     * @param from The address of the current owner of the listing item
     * @param to The address of the new owner of the listing item
     * @param listingItemId The unique ID of the listing item to be transferred
     */
    function _transferListingItemOwnership(address from, address to, uint256 listingItemId) internal {
        // Retrieve the storage listing item
        ListingItem storage item = listingItems[listingItemId];
        // Remove the listing from the old owner's listings
        ListingItem[] storage fromListings = ownerListings[from];
        for (uint256 i = 0; i < fromListings.length; i++) {
            if (fromListings[i].listingItemId == item.listingItemId) {
                fromListings[i] = fromListings[fromListings.length - 1];
                fromListings.pop();
                break;
            }
        }

        // Add the listing to the new owner's listings
        ownerListings[to].push(item);

        // Update the owner field in the ListingItem struct
        item.owner = to;
    }

    /**
     * @notice Function to refund all pending offers except the accepted one
     * @param listingItemId The ID of the listing item
     */
    function _refundPendingOffers(uint256 listingItemId) internal {
        OfferStruct[] storage listingOffers = offers[listingItemId];
        for (uint256 i = 0; i < listingOffers.length; i++) {
            OfferStruct memory offer = listingOffers[i];
            (bool successRefund,) = offer.bidder.call{value: offer.offerAmount}("");
            if (!successRefund) {
                revert Offer__RefundToBidderFailed();
            }
    ...
    /**
     * @notice Function to get all listing items
     * @dev Returns an array of all listing items stored in the contract
     * @return An array of ListingItem structs representing all listings
     */
    function getAllListings() external view returns (ListingItem[] memory) {
        return allListings;
    }

    /**
     * @notice Function to get listing items by owner
     * @dev Returns an array of listing items associated with a specific owner
     * @param owner The address of the owner whose listings are to be retrieved
     * @return An array of ListingItem structs representing the owner's listings
     */
    function getListingsByOwner(address owner) external view returns (ListingItem[] memory) {
        return ownerListings[owner]; // Directly return the listings for the owner
    }
